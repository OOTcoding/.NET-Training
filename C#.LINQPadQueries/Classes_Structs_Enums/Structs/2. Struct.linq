...
// Конструктором по умолчанию в языке C# называется инструкция 
// обнуления значения объекта. С точки зрения CLR конструкторы 
// по умолчанию существуют и язык C# даже умеет их вызывать.
// Язык C# не позволяет создавать пользовательские конструкторы 
// по умолчанию для структур, поскольку это привело бы к падению
// производительности при работе с массивами и существенной путанице.
// Работая на языках, поддерживающих создание конструкторов по умолчанию,
// их объявлять все равно не стоит по тем же причинам, по которым 
// они запрещены в большинстве языков платформы .NET. 
