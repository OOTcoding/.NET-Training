...
//
//3. EE закладывает ccылку на объект в регистр ecx и передает управление конструктору,
...
//методов по умолчанию - ecx и edx (<=DWORD) остальние заталкиваются в стек справа налево) через него передается первый параметр функции. Для экземплярных 
...
// Base Instance Size — это размер объекта, вычисленный загрузчиком классов
// по объявлениям полей в коде. Текущая реализация GC требует,
// чтобы для ее нужд в объекте выделялось минимум 12(24)байтов. 
// Если в классе не определены никакие поля экземпляра, в класс добавляется 4(8) байта. 
// Остальные 8(16) байтов занимают Object Header (который может содержать номер syncblk)
// и TypeHandle. Кроме того, на размер объекта влияет атрибут StructLayoutAttribute.
